...
class GazeboMARATopOrientCollisionv0Env(gazebo_env.GazeboEnv):
    ...
    def __init__(self):
        ...
        """
        #
        self.obs_dim = self.scara_chain.getNrOfJoints() + 9#9#7 #6 hardcode it for now
    ...
    def randomizeObjectType(self, current_obj_name):

       obj = ModelState()
       rospy.wait_for_service('gazebo/get_model_state')
       try:
           obj = self.get_model_state(current_obj_name, '')
    ...
    def take_observation(self):
        ...
        if self.environment['link_names'][-1] is None:
            ...
            # current_quaternion_realnr = current_quaternion[:,:3]
            # tgt_quartenion_realnr = tgt_quartenion[:,:3]
            ...
            d_rot = 2 * np.arccos(np.clip(quat_error[:3], -1., 1.))
    ...
    def step(self, action):
        ...
        Implement the environment step abstraction. Execute action and returns:
            ...
            - done (status)
        """
        # self.randomizeObjectType("target")
        self.iterator+=1
        ...
        # print("self.reward_dist: ", self.reward_dist)
        # careful we have degrees now so we scale with
        orientation_scale = 0.1
        orient_error_array = orientation_scale * self.ob[self.scara_chain.getNrOfJoints()+3:(self.scara_chain.getNrOfJoints()+4)]
        ...
        # self.reward_orient = - orientation_scale * orient_error_array[0]#self.ob[self.scara_chain.getNrOfJoints()+3:(self.scara_chain.getNrOfJoints()+4)]
        ...
        if self._collision_msg is not None and self._collision_msg.collision1_name and self._collision_msg.collision2_name:
            ...
            if abs(self.reward_dist) < 0.01:
                ...
                if abs(self.reward_orient)<0.01:
                    self.reward = (1 + self.reward + self.reward_orient)*2.0
